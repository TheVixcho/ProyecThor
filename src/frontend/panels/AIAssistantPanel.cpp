...
{
    ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(DS::TextPrimary), "Elegí con qué IA querés chatear");
    ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(DS::TextSecondary),
        "Se abre el sitio real de esa IA -- inicia sesión como en cualquier navegador. "
    ...
    if (!m_WebView.IsAvailable()) {
        ...
        ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(DS::TextHint),
            "%s", m_WebView.GetLastError().empty()
                ? "El navegador embebido no está disponible en este sistema -- se abrirá en tu navegador externo."
...
void AIAssistantPanel::RenderBrowserArea()
{
    ...
    if (!m_WebView.IsAvailable()) {
        m_WebView.UpdateBounds(0, 0, 0, 0, false);
        ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(DS::TextSecondary),
            "Se abrió en tu navegador externo (esta ventana no puede embeberlo acá).");
