...
void ImageEditorPanel::Render()
{
    bool visible = false;
    if (m_UIManagerRef)
        visible = DS::BeginGlassPanel("Editor de Imagen", m_UIManagerRef->GetGlassRenderer(),
                                      nullptr, 0, ImVec2(0.0f, 0.0f));
    else
        visible = ImGui::Begin("Editor de Imagen");
    ...
    ProyecThor::Library::DrawIcon_Image(dl, { center.x - iconSz * 0.5f, center.y - iconSz * 1.6f }, iconSz,
                                        ImGui::ColorConvertFloat4ToU32(ImVec4(1, 1, 1, 0.35f)));

    const char* title = "Editor de Imagen";
