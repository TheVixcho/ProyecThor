...
void AudioEditorPanel::Render()
{
    bool visible = false;
    if (m_UIManagerRef)
        visible = DS::BeginGlassPanel("Editor de Audio", m_UIManagerRef->GetGlassRenderer(),
                                      nullptr, 0, ImVec2(0.0f, 0.0f));
    else
        visible = ImGui::Begin("Editor de Audio");
    ...
    ProyecThor::Library::DrawIcon_Audio(dl, { center.x - iconSz * 0.5f, center.y - iconSz * 1.6f }, iconSz,
                                        ImGui::ColorConvertFloat4ToU32(ImVec4(1, 1, 1, 0.35f)));

    const char* title = "Editor de Audio";
